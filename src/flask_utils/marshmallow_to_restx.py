...
def get_restx_params(ma_params: dict):
    ...
    return {"required": ma_params["required"], **restx_params}
...
def get_restx_field(api: Api, ma_field_meta: dict, *, nested: bool = False):
    if nested:
        return restx.fields.Nested(api.model, skip_none=True, **ma_field_meta["params"])
    ...
    restx_field_instance = restx_field(
        required=ma_field_meta["params"].get("required", False), skip_none=True
    )
